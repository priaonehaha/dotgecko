...
{
	internal static class nsIWebBrowserPersistConstants
	{
		/** No special persistence behaviour. */
		internal const UInt32 PERSIST_FLAGS_NONE = 0;
		/** Only use cached data (could result in failure if data is not cached). */
		internal const UInt32 PERSIST_FLAGS_FROM_CACHE = 1;
		/** Bypass the cached data. */
		internal const UInt32 PERSIST_FLAGS_BYPASS_CACHE = 2;
		/** Ignore any redirected data (usually adverts). */
		internal const UInt32 PERSIST_FLAGS_IGNORE_REDIRECTED_DATA = 4;
		/** Ignore IFRAME content (usually adverts). */
		internal const UInt32 PERSIST_FLAGS_IGNORE_IFRAMES = 8;
		/** Do not run the incoming data through a content converter e.g. to decompress it */
		internal const UInt32 PERSIST_FLAGS_NO_CONVERSION = 16;
		/** Replace existing files on the disk (use with due diligence!) */
		internal const UInt32 PERSIST_FLAGS_REPLACE_EXISTING_FILES = 32;
		/** Don't modify or add base tags */
		internal const UInt32 PERSIST_FLAGS_NO_BASE_TAG_MODIFICATIONS = 64;
		/** Make changes to original dom rather than cloning nodes */
		internal const UInt32 PERSIST_FLAGS_FIXUP_ORIGINAL_DOM = 128;
		/** Fix links relative to destination location (not origin) */
		internal const UInt32 PERSIST_FLAGS_FIXUP_LINKS_TO_DESTINATION = 256;
		/** Don't make any adjustments to links */
		internal const UInt32 PERSIST_FLAGS_DONT_FIXUP_LINKS = 512;
		/** Force serialization of output (one file at a time; not concurrent) */
		internal const UInt32 PERSIST_FLAGS_SERIALIZE_OUTPUT = 1024;
		/** Don't make any adjustments to filenames */
		internal const UInt32 PERSIST_FLAGS_DONT_CHANGE_FILENAMES = 2048;
		/** Fail on broken inline links */
		internal const UInt32 PERSIST_FLAGS_FAIL_ON_BROKEN_LINKS = 4096;
		/**
		 ...
		 */
		internal const UInt32 PERSIST_FLAGS_CLEANUP_ON_FAILURE = 8192;
		/**
		 ...
		 * decompress it.
		 */
		internal const UInt32 PERSIST_FLAGS_AUTODETECT_APPLY_CONVERSION = 16384;
		/**
		 * Append the downloaded data to the target file.
		 * This can only be used when persisting to a local file.
		 */
		internal const UInt32 PERSIST_FLAGS_APPEND_TO_FILE = 32768;

		/**
		 * Force relevant cookies to be sent with this load even if normally they
		 * wouldn't be.
		 */
		internal const UInt32 PERSIST_FLAGS_FORCE_ALLOW_COOKIES = 65536;

		/** Persister is ready to save data */
		internal const UInt32 PERSIST_STATE_READY = 1;
		/** Persister is saving data */
		internal const UInt32 PERSIST_STATE_SAVING = 2;
		/** Persister has finished saving data */
		internal const UInt32 PERSIST_STATE_FINISHED = 3;

		/** Output only the current selection as opposed to the whole document. */
		internal const UInt32 ENCODE_FLAGS_SELECTION_ONLY = 1;
		/**
		 * For plaintext output. Convert html to plaintext that looks like the html.
		 * Implies wrap (except inside &lt;pre&gt;), since html wraps.
		 * HTML output: always do prettyprinting, ignoring existing formatting.
		 */
		internal const UInt32 ENCODE_FLAGS_FORMATTED = 2;
		/**
		 ...
		 */
		internal const UInt32 ENCODE_FLAGS_RAW = 4;
		/** Output only the body section, no HTML tags. */
		internal const UInt32 ENCODE_FLAGS_BODY_ONLY = 8;
		/** Wrap even if when not doing formatted output (e.g. for text fields). */
		internal const UInt32 ENCODE_FLAGS_PREFORMATTED = 16;
		/** Wrap documents at the specified column. */
		internal const UInt32 ENCODE_FLAGS_WRAP = 32;
		/**
		 ...
		 */
		internal const UInt32 ENCODE_FLAGS_FORMAT_FLOWED = 64;
		/** Convert links to absolute links where possible. */
		internal const UInt32 ENCODE_FLAGS_ABSOLUTE_LINKS = 128;
		 ...
		 * do what you want.
		 */
		internal const UInt32 ENCODE_FLAGS_ENCODE_W3C_ENTITIES = 256;
		 ...
		 */
		internal const UInt32 ENCODE_FLAGS_CR_LINEBREAKS = 512;
		/**
		 ...
		 * default format is used.
		 */
		internal const UInt32 ENCODE_FLAGS_LF_LINEBREAKS = 1024;
		/** For plaintext output. Output the content of noscript elements. */
		internal const UInt32 ENCODE_FLAGS_NOSCRIPT_CONTENT = 2048;
		/** For plaintext output. Output the content of noframes elements. */
		internal const UInt32 ENCODE_FLAGS_NOFRAMES_CONTENT = 4096;
